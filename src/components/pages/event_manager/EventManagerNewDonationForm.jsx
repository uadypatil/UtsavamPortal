...
const INITIAL_DONOR = { fullName: '', contactNumber: '', email: '', address: '' };
...
function EventManagerNewDonationForm() {
    ...
    const validate = () => {
        ...
        if (!donation.amount || Number(donation.amount) <= 0) next.amount = 'Enter a valid amount';
        if (!donation.paymentMode) next.paymentMode = 'Required';
        if (donation.paymentMode === 'upi' && !donation.transactionId.trim()) next.transactionId = 'Required for UPI payments';
        if (!consentGiven) next.consent = 'Donor consent must be confirmed before saving this donation.';
        return next;
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (donor.contactNumber.trim()) {
                const matches = await donorsApi.list({ search: donor.contactNumber.trim() });
                const list = Array.isArray(matches) ? matches : matches?.items || [];
                const existing = list.find((d) => d.phone === donor.contactNumber.trim());
            ...
            if (!donorId) {
                const created = await donorsApi.create({
                    name: donor.fullName.trim(),
                    phone: donor.contactNumber.trim(),
            ...
            toast.error(apiErrorMessage(error, 'Unable to record this donation. Please try again.'));
        } finally {
            setSubmitting(false);
        }
    };
                    ...
                    <form onSubmit={handleSubmit} style={{ height: "75dvh", overflowY: "scroll" }}>
                        ...
                        <div className="card mb-4">
                            <div className="card-body pb-2">
                                ...
                                <input type="text" name="fullName" value={donor.fullName} onChange={handleDonorChange} className={`form-control ${errors.fullName ? 'is-invalid' : ''}`} />
                                {errors.fullName && <div className="invalid-feedback d-block">{errors.fullName}</div>}

                                <label className='form-label mt-3'>Contact Number <span className='text-muted'>(optional, used to avoid duplicate donor records)</span></label>
                                <input type="text" name="contactNumber" value={donor.contactNumber} onChange={handleDonorChange} className='form-control' />

                                <label className='form-label mt-3'>Email <span className='text-muted'>(for receipt)</span></label>
                                <input type="email" name="email" value={donor.email} onChange={handleDonorChange} className='form-control' />

                                <label className='form-label mt-3'>Address<span className='text-danger'>*</span></label>
                                <input type="text" name="address" value={donor.address} onChange={handleDonorChange} className={`form-control ${errors.address ? 'is-invalid' : ''}`} />
                        ...
                        <div className="card mb-4">
                            <div className="card-body pb-2">
                                ...
                                <input type="number" name="amount" value={donation.amount} onChange={handleDonationChange} className={`form-control ${errors.amount ? 'is-invalid' : ''}`} />
                                ...
                                <select name="paymentMode" value={donation.paymentMode} onChange={handleDonationChange} className={`form-select ${errors.paymentMode ? 'is-invalid' : ''}`}>
                                    <option value="" disabled>Select payment mode</option>
                                    <option value="cash">Cash</option>
                                    <option value="upi">UPI</option>
                                </select>
                                {errors.paymentMode && <div className="invalid-feedback d-block">{errors.paymentMode}</div>}

                                {donation.paymentMode === 'upi' && (
                                    <div className='mt-3'>
                                        <button type="button" className='btn btn-primary mb-2' onClick={() => setShowQR((v) => !v)}>
                                            {showQR ? 'Hide QR' : 'Show Payment QR'}
                                        </button>
                                        {showQR && <img src={qrcodeImg} alt="Mandal UPI QR" className='img-fluid' />}

                                        <label className='form-label mt-3'>Transaction ID<span className='text-danger'>*</span></label>
                                        <input type="text" name="transactionId" value={donation.transactionId} onChange={handleDonationChange} className={`form-control ${errors.transactionId ? 'is-invalid' : ''}`} />
                                        {errors.transactionId && <div className="invalid-feedback d-block">{errors.transactionId}</div>}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className={`card mb-4 ${errors.consent ? 'border-danger' : ''}`}>
                            <div className="card-body pb-2">
                                <div className="form-check">
                                    <input
                                        ...
                                        onChange={(e) => { setConsentGiven(e.target.checked); setErrors((p) => ({ ...p, consent: '' })); }}
                                    />
                                    <label className="form-check-label small" htmlFor="donorConsent">
                                        I confirm the donor has been informed that their name, contact number, email
                                        and address are being collected for donation, receipt and accounting
                                        purposes, and that they consent to this in accordance with the{' '}
                        ...
                        <div className='mt-4'>
                            <button type='submit' className='btn btn-success w-100' disabled={submitting}>
                                {submitting ? (
                                    <><span className="spinner-border spinner-border-sm me-2" />Saving Donation...</>
                                ) : 'Donate'}
                            </button>
                        </div>
                    </form>
