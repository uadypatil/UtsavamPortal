...
const STATUS_BADGE = {
    ...
    REVISION_REQUESTED: { label: 'Revision Requested', className: 'ep-status--pending' },
};
...
export default function ExpenseManager() {
    ...
    const load = async () => {
        ...
        try {
            const [expenseData, categoryData] = await Promise.all([
                expensesApi.list(eventId ? { eventId } : undefined),
                expenseCategoriesApi.list(),
            ]);
            setExpenses(Array.isArray(expenseData) ? expenseData : expenseData?.items || []);
            setCategories(Array.isArray(categoryData) ? categoryData : categoryData?.items || []);
        } catch (error) {
    ...
    const openCreate = () => {
        setForm({ title: '', categoryId: '', amount: '', description: '' });
        setFormErrors({});
        setModalMode('create');
    };
    ...
    const handleSubmitForApproval = async (expense) => {
        const ok = await confirm({
            ...
            message: 'Once submitted, you will not be able to edit it unless a Super Admin requests a revision.',
            confirmText: 'Submit',
            variant: 'success',
        });
        if (!ok) return;
        ...
        <div className="wrap-content h-auto w-100 border-3 border-secondary shadow rounded-5 p-3 p-md-4">
            <div className="ep-festive-banner mb-4">
                <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
                    <div>
                        <h4 className="fw-bold mb-1 position-relative"><i className="bi bi-receipt-cutoff me-2" />Expense Management</h4>
                        <p className="mb-0 opacity-90 position-relative">Track festival expenses from draft through approval and payment.</p>
                    </div>
                    <button className="btn ep-action-btn ep-action-btn--indigo" onClick={openCreate} disabled={categories.length === 0}>
            ...
            {categories.length === 0 && !loading && (
                <div className="alert alert-warning">
                    ...
                    No expense categories exist yet — a Super Admin needs to create at least one before expenses can be filed.
                </div>
            )}

            <div className="ep-chart-card">
                {loading ? (
                    <div className="py-5 text-center text-muted"><span className="spinner-border spinner-border-sm me-2" />Loading expenses...</div>
                ) : loadError ? (
                    ...
                    <div className="table-responsive">
                        <table className="table align-middle">
                            ...
                            <tbody>
                                {expenses.map((exp) => {
                                    const badge = STATUS_BADGE[exp.status] || { label: exp.status, className: 'ep-status--disabled' };
                                    ...
                                    return (
                                        <tr key={exp.id}>
                                            ...
                                            <td>{exp.categoryName || categoryName(exp.categoryId)}</td>
                                            <td>₹{exp.amount}</td>
                                            <td><span className={`ep-status ${badge.className}`}><span />{badge.label}</span></td>
                                            <td>
                                                {isApproved ? (
                                                    <select
                                                        ...
                                                        disabled={busy}
                                                        onChange={(e) => handlePaymentStatus(exp, e.target.value)}
                                                    >
                                            ...
                                            <td className="text-end">
                                                <div className="d-flex justify-content-end gap-2">
                                                    {isDraft && (
                                                        <>
                                                            <button className="btn ep-icon-btn ep-icon-btn--edit" title="Edit draft" onClick={() => openEdit(exp)} disabled={busy}>
                                                                <i className="bi bi-pencil" />
                                                            </button>
                                                            <button className="btn ep-action-btn ep-action-btn--indigo btn-sm" onClick={() => handleSubmitForApproval(exp)} disabled={busy}>
                                                                {busy ? 'Submitting...' : 'Submit'}
                                                            </button>
                                                        </>
                                                    )}
                                                    {exp.status === 'REVISION_REQUESTED' && (
                                                        <button className="btn ep-icon-btn ep-icon-btn--edit" title="Revise and resubmit" onClick={() => openEdit(exp)} disabled={busy}>
                                                            <i className="bi bi-pencil" />
                                                        </button>
                                                    )}
                                                </div>
                    ...
                    <div className="ep-modal">
                        <div className="ep-modal__header">
                            <div className="d-flex align-items-center gap-2">
                                <div className="ep-modal-icon"><i className="bi bi-receipt-cutoff" /></div>
                                <h5 className="fw-bold mb-0">{modalMode === 'create' ? 'New Expense (Draft)' : 'Edit Draft Expense'}</h5>
                            </div>
                        ...
                        <form onSubmit={submit} noValidate>
                            <div className="ep-modal__body">
                                <div className="row g-3">
                                    <div className="col-12">
                                        <label className="form-label fw-semibold">Title<span className="text-danger ms-1">*</span></label>
                                        <input className={`form-control ${formErrors.title ? 'is-invalid' : ''}`} value={form.title || ''} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} />
                                        {formErrors.title && <div className="invalid-feedback d-block">{formErrors.title}</div>}
                                    </div>
                                    <div className="col-md-6">
                                        <label className="form-label fw-semibold">Category<span className="text-danger ms-1">*</span></label>
                                        <select className={`form-select ${formErrors.categoryId ? 'is-invalid' : ''}`} value={form.categoryId || ''} onChange={(e) => setForm((p) => ({ ...p, categoryId: e.target.value }))}>
                                    ...
                                    <div className="col-md-6">
                                        <label className="form-label fw-semibold">Amount (₹)<span className="text-danger ms-1">*</span></label>
                                        <input type="number" min="0" step="0.01" className={`form-control ${formErrors.amount ? 'is-invalid' : ''}`} value={form.amount ?? ''} onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} />
                            ...
                            <div className="ep-modal__footer">
                                ...
                                <button type="submit" className="btn ep-action-btn ep-action-btn--indigo" disabled={saving}>
                                    {saving ? 'Saving...' : modalMode === 'create' ? 'Save Draft' : 'Save Changes'}
                                </button>
                            </div>
                        </form>
                    </div>
