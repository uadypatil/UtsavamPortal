...
export default function EventsList() {
    ...
    const filtered = useMemo(() => {
        ...
        if (!q) return events;
        return events.filter((e) => (e.event_name || e.name || '').toLowerCase().includes(q));
    }, [events, search]);
    ...
    const handleToggleStatus = async (event) => {
        const isActive = event.status === STATUS_VALUES.active;
        const ok = await confirm({
        ...
        setActionLoadingId(event.id);
        try {
            await eventsApi.setStatus(event.id, isActive ? STATUS_VALUES.inactive : STATUS_VALUES.active);
    ...
    return (
        <div className="wrap-content h-auto w-100 border-3 border-secondary shadow rounded-5 p-3 p-md-4">
            <div className="ep-festive-banner mb-4">
                <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
                    <div>
                        ...
                        <p className="mb-0 opacity-90 position-relative">Manage every festival/event your Mandal runs — not limited to one.</p>
                    </div>
                    <button className="btn ep-action-btn ep-action-btn--indigo" onClick={() => navigate('/admin/events/new')}>
            ...
            <div className="ep-chart-card">
                <div className="row g-3 align-items-end mb-4">
                    <div className="col-lg-8 col-md-6">
                        ...
                        <div className="ep-search-box">
                            ...
                            <input className="form-control" placeholder="Search by event name..." value={search} onChange={(e) => setSearch(e.target.value)} />
                ...
                {loading ? (
                    <div className="py-5 text-center text-muted"><span className="spinner-border spinner-border-sm me-2" />Loading events...</div>
                ) : loadError ? (
                    ...
                    <div className="text-center py-5 text-muted">
                        ...
                        {events.length === 0 ? 'No events yet — create your first festival event.' : 'No events match your search.'}
                    </div>
                ) : (
                        ...
                        <table className="table align-middle">
                            ...
                            <tbody>
                                {filtered.map((event) => {
                                    ...
                                    return (
                                        <tr key={event.id}>
                                            <td className="fw-semibold">{event.event_name || event.name}</td>
                                            <td>{event.event_date_from || event.startDate || '—'}</td>
                                            <td>{event.event_date_to || event.endDate || '—'}</td>
                                            <td>
                                            ...
                                            <td className="text-end">
                                                <div className="d-flex justify-content-end gap-2">
                                                    <button className="btn ep-icon-btn ep-icon-btn--edit" title="Edit" onClick={() => navigate(`/admin/events/${event.id}/edit`)} disabled={busy}>
                                                    ...
                                                    <button className="btn ep-icon-btn ep-icon-btn--delete" title="Delete" onClick={() => handleDelete(event)} disabled={busy}>
